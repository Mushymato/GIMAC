...
public class ScheduleUtilityFunctions(IMonitor monitor, ITranslationHelper translation)
{
    /// <summary>
    /// Regex for a schedulepoint format.
    /// </summary>
    public static readonly Regex ScheduleRegex =
        new(
    ...
    /// </summary>
    public static readonly Regex BedRegex =
        new(
            // <time> bed
    ...
    public bool TryFindGOTOschedule(NPC npc, SDate date, string rawData, out string scheduleString)
    {
        ...
        string[] splits = rawData.Split(separator: '/', count: 3, options: StringSplitOptions.TrimEntries);
        ...
        switch (command[0])
        {
            case "GOTO":
                ...
                if (npc.hasMasterScheduleEntry(newKey))
                {
                    ...
                    if (newScheduleKey.Equals(rawData, StringComparison.Ordinal))
                    {
                        monitor.Log(
                            translation
                                .Get("GOTO_INFINITE_LOOP")
                                .Default("Infinite loop detected, skipping this schedule."),
                            LogLevel.Warn
                        );
                        return false;
                    }
            ...
            case "NOT":
                ...
                if (command[1].Equals("friendship", StringComparison.Ordinal))
                {
                    ...
                    if (friendNpc is null)
                    {
                        ...
                        monitor.Log(
                            translation
                                .Get("GOTO_FRIEND_NOT_FOUND")
                                .Default(
                                    "NPC {{npc}} not found, friend requirement {{requirment}} cannot be evaluated: {{scheduleKey}}"
                                )
                                .Tokens(
                                    new
                                    {
                    ...
                    if (!int.TryParse(command[3], out int heartLevel))
                    {
                        ...
                        monitor.Log(
                            translation
                                .Get("GOTO_ILL_FORMED_FRIENDSHIP")
            ...
            case "MAIL":
                // MAIL mailkey
                return
                    Game1.MasterPlayer.mailReceived.Contains(command[1])
                    || NetWorldState.checkAnywhereForWorldStateID(command[1])
    ...
    )
    {
        ...
        foreach (
            string schedulepoint in schedule.Split(
                '/',
        ...
        )
        {
            try
            {
                ...
                if (!match.Success)
                {
                    ...
                    if (bedmatch.Success)
                    {
                        Dictionary<string, string> bedmatchDict = bedmatch.MatchGroupsToDictionary(
                            (string key) => key,
                        ...
                        string bedtime =
                            (
                                bedmatchDict.TryGetValue("arrival", out string? bedarrival)
                                && bedarrival.Equals("a", StringComparison.OrdinalIgnoreCase)
                            )
                                ? bedarrival + bedmatchDict["time"]
                                : bedmatchDict["time"];
                        if (npc.isMarried() || npc.DefaultMap.Equals("FarmHouse", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            npc.TryGetScheduleEntry("default", out string? defaultSchedule)
                        ...
                        )
                        {
                            ...
                            npc.TryGetScheduleEntry("spring", out string? springSchedule)
                    ...
                    if (int.TryParse(originaltime, out int path2bedtime))
                    {
                        if (path2bedtime < lasttime)
                        {
                            ...
                            { // I've already adjusted the last time parameter to account for travel time
                                path2bedtime = Utility.ConvertMinutesToTime(
                                    ((Utility.ConvertTimeToMinutes(lasttime) * 10) / 10) + 10
                                );
                            }
                            else if (
                                remainderSchedule.TryGetValue(lasttime, out SchedulePathDescription? lastschedpoint)
                            )
                            {
                                path2bedtime = Utility.ConvertMinutesToTime(
                                    (
                                        (Utility.ConvertTimeToMinutes(lasttime) + lastschedpoint.GetExpectedRouteTime())
                                        * 10
                                        / 10
                                    ) + 10
                                );
                        ...
                        originaltime.Length > 0
                        && originaltime.StartsWith('a')
                        && int.TryParse(originaltime[1..], out path2bedtime)
                    )
                    {
                ...
                Dictionary<string, string> matchDict = match.MatchGroupsToDictionary(
                    (key) => key,
                    (value) => value.Trim()
                );
                ...
                if (!Game1.isLocationAccessible(location))
                {
                    if (npc.TryGetScheduleEntry(location + "_Replacement", out string? replacement))
                    {
                        ...
                        if (
                            !replacementdata.MoveNext()
                            || !int.TryParse(replacementdata.Current, out x)
                            || !replacementdata.MoveNext()
                            || !int.TryParse(replacementdata.Current, out y)
                        )
                        {
                    ...
                    else
                    {
                        if (enforceStrictTiming)
                        {
                            monitor.Log(
                                translation
                                    .Get("NO_REPLACEMENT_LOCATION")
                                    .Default(
                                        "Location replacement for {{location}} requested but not found for {{npc}}"
                                    )
                ...
                else if (time <= lasttime)
                {
                    monitor.Log(
                        translation
                            .Get("TOO_TIGHT_TIMELINE")
                            .Default(
                                "{{time}} position in schedule {{scheduleKey}} for {{npc}} is too tight. Will be skipped."
                            )
                            .Tokens(
                                new
                                {
                ...
                if (
                    matchDict.TryGetValue("arrival", out string? arrival)
                    && arrival.Equals("a", StringComparison.OrdinalIgnoreCase)
                )
                {
