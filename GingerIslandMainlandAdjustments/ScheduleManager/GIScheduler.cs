...
internal static class GIScheduler
{
    ...
    internal static void GenerateAllSchedules()
    {
        ...
        if (
            Game1.getLocationFromName("IslandSouth") is not IslandSouth island
            || !island.resortRestored.Value
            || !island.resortOpenToday.Value
            || Game1.IsRainingHere(island)
        ...
        if (explorers?.Count > 0 && explorerGroupName is not null)
        {
            Globals.ModMonitor.DebugOnlyLog(
                $"Found explorer group: {string.Join(", ", explorers.Select((NPC npc) => npc.Name))}."
            );
        ...
        if (
            Globals.Config.Capacity == 0
            && (Globals.SaveDataModel is null || Globals.SaveDataModel.NPCsForTomorrow.Count == 0)
        )
        {
            ...
            Globals.ModMonitor.Log(
                $"GI Scheduler did not need to run, took {stopwatch.Elapsed.TotalMilliseconds:F2} ms anyways",
                LogLevel.Info
            );
#endif
            return;
        ...
        Dictionary<string, string> animationDescriptions = Globals.GameContentHelper.Load<Dictionary<string, string>>(
    ...
    private static (HashSet<NPC> group, string groupname)? GenerateExplorerGroup(Random random)
    {
        if (random.OfChance(Globals.Config.ExplorerChance) && ExplorerGroups.Count > 0)
        {
            ...
            if (explorerGroups.Count > 0)
            {
                ...
                CurrentAdventurers = ExplorerGroups[CurrentAdventureGroup]
                    .Where(IslandSouth.CanVisitIslandToday)
                    .Take(3)
                    .ToHashSet();
    ...
    private static List<NPC> GenerateVisitorList(Random random, int capacity, HashSet<NPC>? explorers)
    {
        ...
        if (random.OfChance(Globals.Config.GroupChance))
        {
            ...
            foreach (string key in IslandGroups.Keys)
            {
                ...
                if (
                    (IslandGroups[key].Count <= capacity - visitors.Count || key == "allSpouses")
        ...
        if (
            NPCCache.GetByVillagerName("Gus") is NPC gus
            && !visitors.Contains(gus)
            && valid_visitors.Contains(gus)
            && Globals
                .Config.GusDayAsShortString()
                .Equals(Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth), StringComparison.OrdinalIgnoreCase)
        ...
        int kidsremoved = valid_visitors.RemoveWhere(
            (NPC npc) =>
                npc.Age == NPC.child
                && (
                    !IslandSouthPatches.Exclusions.TryGetValue(npc, out string[]? exclusions)
                    || !exclusions.Contains("freerange")
                )
        );
        int neveralone = valid_visitors.RemoveWhere(
            (NPC npc) =>
                IslandSouthPatches.Exclusions.TryGetValue(npc, out string[]? exclusions)
        ...
        {
            Globals.ModMonitor.Log(
                $"Excluded {kidsremoved} kids and {neveralone} never alone villagers from the valid villagers list"
            );
        }
...

        {
            ...
            if (
                visitors.Any((NPC npc) => npc.Name.Equals("George", StringComparison.OrdinalIgnoreCase))
            ...
            )
            {
                ...
                {
                    if (
                        !visitors[i].Name.Equals("Gus", StringComparison.OrdinalIgnoreCase)
                        && !visitors[i].Name.Equals("George", StringComparison.OrdinalIgnoreCase)
                    )
                    {
...

        {
            ...
            if (
                visitors.FirstOrDefault((NPC npc) => npc.Name.Equals("George", StringComparison.OrdinalIgnoreCase))
                    is NPC george
                && visitors.FirstOrDefault((NPC npc) => npc.Name.Equals("Evelyn", StringComparison.OrdinalIgnoreCase))
                    is NPC evelyn
            )
            {
        ...
        Globals.ModMonitor.DebugOnlyLog(
            $"{visitors.Count} visitors: {string.Join(", ", visitors.Select((NPC npc) => npc.Name))}"
        );
        IslandSouthPatches.ClearCache();
            ...
            if (musicians.Count > 0)
            {
                musician =
                    visitors
                        .Where(
                            (NPC npc) =>
                                musicians.Contains(npc)
                                && animationDescriptions.ContainsKey($"{npc.Name.ToLowerInvariant()}_beach_towel")
                        )
                        .OrderBy(_ => Random.Shared.Next())
                        .FirstOrDefault() ?? musician;
            }
        }
    ...
    private static List<GingerIslandTimeSlot> AssignIslandSchedules(
        Random random,
        List<NPC> visitors,
    ...
    )
    {
        ...
        List<GingerIslandTimeSlot> activities = TIMESLOTS
            .Select((i) => new GingerIslandTimeSlot(i, Bartender, Musician, random, visitors))
            .ToList();
    ...
    private static Dictionary<NPC, string> RenderIslandSchedules(
        Random random,
        List<NPC> visitors,
        List<GingerIslandTimeSlot> activities
    )
    {
        ...
        foreach (NPC visitor in visitors)
        {
            ...
            if (
                Globals.Config.StageFarNpcsAtSaloon
                && visitor.getSpouse() is null
                && Game1.getLocationFromName(visitor.DefaultMap) is not FarmHouse
            )
            {
                try
                {
                    string[]? maplist = WarpPathfindingCache.GetLocationRoute(
                        visitor.DefaultMap,
                        "IslandSouth",
                        visitor.Gender
                    );
                    ...
                    {
                        Globals.ModMonitor.Log(
                            $"{visitor.Name} has a long way to travel, so staging them at the Saloon."
                        );
                        new SchedulePoint(
                            random: random,
                            npc: visitor,
            ...
            sb.Clear();
            Globals.ModMonitor.DebugOnlyLog(
                $"For {visitor.Name}, created island schedule {completedSchedules[visitor]}"
            );
        }
