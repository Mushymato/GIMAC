...
internal static class ScheduleUtilities
{
    ...
    private static readonly Dictionary<
        string,
        (string key, Dictionary<int, SchedulePathDescription> schedule)
    > Schedules = new();
    ...
    internal static string? FindProperGISchedule(NPC npc, SDate date, out string? key)
    {
        ...
        if (
            npc.hasMasterScheduleEntry(key)
            && Globals.UtilitySchedulingFunctions.TryFindGOTOschedule(
                npc,
                date,
                npc.getMasterScheduleEntry(key),
                out string? scheduleEntry
            )
        ...
        // GIREmainder
        if (
            npc.hasMasterScheduleEntry(scheduleKey)
            && Globals.UtilitySchedulingFunctions.TryFindGOTOschedule(
                npc,
                date,
                npc.getMasterScheduleEntry(scheduleKey),
                out scheduleEntry
            )
            && scheduleEntry.StartsWith(POST_GI_START_TIME)
        )
    ...
    internal static bool ParseMasterScheduleAdjustedForChild2NPC(NPC npc, string key, string rawData)
    {
        if (Globals.IsChildToNPC?.Invoke(npc) == true)
        {
            ...
            {
                Dictionary<int, SchedulePathDescription>? schedule = Globals.UtilitySchedulingFunctions.ParseSchedule(
                    key,
                    scheduleString,
                    npc,
                    "BusStop",
            ...
            (
                npc.DefaultMap.Equals("FarmHouse", StringComparison.Ordinal)
                || npc.DefaultMap.Contains("Cabin", StringComparison.Ordinal)
            ) && !npc.isMarried()
        )
        {
        ...
        else
        {
            ...
            if (
                !rawData.StartsWith("0 ")
                && npc.DefaultPosition != Vector2.Zero
                && (npc.currentLocation.Name != npc.DefaultMap || npc.DefaultPosition != npc.Position)
            )
            {
                ...
                if (location is null)
                {
                    Globals.ModMonitor.Log(
                        $"NPC {npc.Name} has default map {npc.DefaultMap} which could not be found!",
                        LogLevel.Warn
                    );
                    return false;
                }
    ...
    internal static void FixUpSchedules()
    {
        ...
        {
            if (
                npc.Schedule is null
                && Globals.IsChildToNPC?.Invoke(npc) == true
                && ScheduleUtilities.Schedules.TryGetValue(
                    npc.Name,
                    out (string key, Dictionary<int, SchedulePathDescription> schedule) pair
                )
            )
            {
                Globals.ModMonitor.Log(
                    $"Fixing up schedule for {npc.Name}, which appears to have been nulled.",
                    LogLevel.Warn
                );
