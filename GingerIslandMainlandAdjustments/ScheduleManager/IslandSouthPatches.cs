...
internal static class IslandSouthPatches
{
    ...
    internal static Dictionary<NPC, string[]> Exclusions => exclusions ??= AssetLoader.GetExclusions();
    ...
    private static void ExtendCanGoToIsland(NPC npc, ref bool __result)
    {
        try
        {
            if (!__result)
            {
                ...
                if (
                    (
                        Globals.Config.AllowSandy == VillagerExclusionOverride.Yes
                ...
                )
                {
                    ...
                    && (
                        ...
                        || npc.Name.Equals("Evelyn", StringComparison.OrdinalIgnoreCase)
                    )
                )
                {
            ...
            if (Globals.Config.RequireResortDialogue && !npc.Dialogue.ContainsKey("Resort"))
            {
                Globals.ModMonitor.Log(
                    $"{npc.Name} appears to lack resort dialogue, removing from pool.",
                    LogLevel.Info
                );
                __result = false;
                return;
            ...
            */
            ...
            {
                switch (
                    Globals.Config.ScheduleStrictness.TryGetValue(npc.Name, out ScheduleStrictness strictness)
                        ? strictness
                ...
                )
                {
                    ...
                    {
                        if (
                            !Exclusions.TryGetValue(npc, out string[]? exclusions)
                            || !exclusions.Any(
                                static (a) => a.Equals("AllowOnSpecialDays", StringComparison.OrdinalIgnoreCase)
                            )
                        )
                        {
            ...
            {
                if (
                    (int.TryParse(condition, out int day) && day == Game1.dayOfMonth)
                    || Game1.currentSeason.Equals(condition, StringComparison.OrdinalIgnoreCase)
                    || Game1
                        .shortDayNameFromDayOfSeason(Game1.dayOfMonth)
                        .Equals(condition, StringComparison.OrdinalIgnoreCase)
                    || $"{Game1.currentSeason}_{Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth)}".Equals(
                        condition,
                    ...
                    || (
                        !Globals.Config.UseThisScheduler
                        && "neveralone".Equals(condition, StringComparison.OrdinalIgnoreCase)
                    )
                )
                {
                    __result = false;
            ...
            switch (Globals.Config.WearIslandClothing)
            {
                ...
                case WearIslandClothing.All:
                    if (character.Name.Equals("Lewis", StringComparison.OrdinalIgnoreCase))
                    {
                        try
                        {
                            Game1.temporaryContent.Load<Texture2D>(
                                $"Characters\\{NPC.getTextureNameForCharacter(character.Name)}_Beach"
                            );
                            __result = true;
                            return false;
                        }
