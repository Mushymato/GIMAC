...
internal static class MidDayScheduleEditor
{
    ...
    {
        if (!Context.IsMainPlayer || e.NewTime >= 900 || Globals.Config.UseThisScheduler)
        ...
        {
            if (
                name.Equals("Gus", StringComparison.OrdinalIgnoreCase) // Gus runs saloon, skip.
    ...
    internal static bool AdjustSpecificSchedule(NPC npc)
    {
        ...
        if (
            ScheduleUtilities.FindProperGISchedule(npc, SDate.Now(), out string? key) is not string schedule
            || key is null
        )
        {
            return false;
    ...
    internal static Dictionary<int, SchedulePathDescription>? ParseGIRemainderSchedule(
        string scheduleKey,
        string? schedule,
        NPC npc
    )
    {
        ...
        return Globals.UtilitySchedulingFunctions.ParseSchedule(
            scheduleKey,
            schedule,
            npc,
            GIMap,
