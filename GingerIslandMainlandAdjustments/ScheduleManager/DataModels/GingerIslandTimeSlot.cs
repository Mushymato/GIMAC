...
internal class GingerIslandTimeSlot
{
    ...
    /// </summary>
    private static readonly Point[] DanceDeltas =
    [
        new Point(1, 1),
        new Point(-1, -1),
        new Point(2, 0),
    ...
    /// </summary>
    private static readonly PossibleIslandActivity Drinking =
        new(
            possiblepoints: [new Point(12, 23), new Point(15, 23)],
    ...
    private static readonly PossibleIslandActivity IslandNorth =
        new(
            possiblepoints: [new Point(33, 83), new Point(34, 73), new Point(48, 81), new Point(52, 73)],
            map: "IslandNorth",
            basechance: 0.5,
            ...
            chanceMap: (NPC npc) =>
                npc.Age == NPC.adult
                && npc.Optimism == NPC.positive
                && !npc.Name.Equals("George", StringComparison.OrdinalIgnoreCase)
                    ? 0.5
                    : 0
        );
    ...
    private readonly Dictionary<NPC, SchedulePoint> assignments = [];
    ...
    private readonly Dictionary<NPC, string> animations = [];
    ...
    internal Dictionary<NPC, string> AssignActivities(
        Dictionary<NPC, string> lastAssignment,
        Dictionary<string, string> animationDescriptions
    )
    {
        // Get a list of possible dancers (who have _beach_dance as a possible animation).
        HashSet<NPC> dancers =
            (this.musician is not null)
                ? this
                    .visitors.Where(
                        (NPC npc) => animationDescriptions.ContainsKey($"{npc.Name.ToLowerInvariant()}_beach_dance")
                    )
                    .ToHashSet()
                : [];
        ...
        if (this.bartender is not null)
        {
            {
                string? varkey =
                    GIScheduler.CurrentVisitingGroup?.Contains(this.bartender) == true
                        ? $"Resort_Bartend_{GIScheduler.CurrentGroup}"
                        : null;
                this.AssignSchedulePoint(
                    this.bartender,
                    new SchedulePoint(
                        random: this.random,
                        npc: this.bartender,
            ...
            {
                if (
                    !this.assignments.ContainsKey(possibledrinker)
                    && possibledrinker.Age != NPC.child
                    && !dancers.Contains(possibledrinker)
                    && possibledrinker != this.musician
                )
                {
                    SchedulePoint? schedulePoint = Drinking.TryAssign(
                        ...
                        groupName: GIScheduler.CurrentVisitingGroup?.Contains(possibledrinker) == true
                            ? GIScheduler.CurrentGroup
                            : null
                    );
        ...
        if (this.musician is not null && !this.assignments.ContainsKey(this.musician))
        {
            SchedulePoint? musicianPoint = Music.TryAssign(
                ...
                animation_descriptions: animationDescriptions,
                groupName: GIScheduler.CurrentVisitingGroup?.Contains(this.musician) == true
                    ? GIScheduler.CurrentGroup
                    : null
            );
            if (musicianPoint is not null)
            {
                ...
                PossibleIslandActivity closeDancePoint =
                    new(
                        DanceDeltas
                            .Select((Point pt) => new Point(musician_loc.X + pt.X, musician_loc.Y + pt.Y))
                            .ToArray(),
                        basechance: 0.7,
                ...
                {
                    SchedulePoint? dancerPoint =
                        closeDancePoint.TryAssign(
                            random: this.random,
                            character: dancer,
                            time: this.timeslot,
                            usedPoints: this.usedPoints,
                            lastAssignment: lastAssignment,
                            animation_descriptions: animationDescriptions,
                            groupName: GIScheduler.CurrentVisitingGroup?.Contains(dancer) == true
                                ? GIScheduler.CurrentGroup
                                : null
                        )
                        ?? Dance.TryAssign(
        ...
        foreach (NPC visitor in this.visitors)
        {
            ...
            foreach (PossibleIslandActivity possibleIslandActivity in PossibleActivities)
            {
                SchedulePoint? schedulePoint = possibleIslandActivity.TryAssign(
                    ...
                    lastAssignment: lastAssignment,
                    animation_descriptions: animationDescriptions,
                    groupName: GIScheduler.CurrentVisitingGroup?.Contains(visitor) == true
                        ? GIScheduler.CurrentGroup
                        : null
                );
                if (schedulePoint is not null)
            ...
            Globals.ModMonitor.DebugOnlyLog(
                $"Now using fall back spot assignment for {visitor.Name} at {this.timeslot}",
                LogLevel.Warn
            );
            ...
            {
                SchedulePoint? schedulePoint = PossibleActivities[i]
                    .TryAssign(
                        random: this.random,
                        character: visitor,
                        time: this.timeslot,
            ...
            Globals.ModMonitor.DebugOnlyLog(
                $"Warning: No activity found for {visitor.Name} at {this.timeslot}",
                LogLevel.Warn
            );
            CONTINUELOOP:
            ;
    ...
    private static PossibleIslandActivity[] GenerateIslandActivtyList()
    {
        ...
        [
            ...
            new PossibleIslandActivity(
                possiblepoints: [new Point(14, 27), new Point(17, 28), new Point(20, 27), new Point(23, 28)],
                basechance: 0.6,
            ...
            // dancing
            new PossibleIslandActivity(
                possiblepoints: [new Point(18, 26), new Point(21, 30), new Point(25, 25)],
            ...
            // wandering
            new PossibleIslandActivity(
                possiblepoints: [new Point(7, 16), new Point(31, 24), new Point(18, 13), new Point(24, 15)],
                basechance: 0.4,
            ...
            // fishing
            new PossibleIslandActivity(
                possiblepoints: [new Point(21, 44)],
#if DEBUG
                basechance: 1,
#else
                basechance: 0.4,
            ...
            // under umbrella
            new PossibleIslandActivity(
                possiblepoints: [new Point(26, 26), new Point(28, 29), new Point(10, 27)],
                chanceMap: static (NPC npc) =>
                    npc.Name.Equals("Abigail", StringComparison.OrdinalIgnoreCase) ? 0.5 : 0.3,
            ...
            // sitting on chair
            new PossibleIslandActivity(
                possiblepoints: [new Point(20, 24), new Point(30, 29)],
                dialogueKey: "Resort_Chair",
                basechance: 0.6,
                chanceMap: static (NPC npc) =>
                    (npc.Age == NPC.adult || (npc.Age == NPC.teen && npc.SocialAnxiety == NPC.shy)) ? 0.6 : 0,
            ...
            new PossibleIslandActivity(
                ...
                basechance: 0,
                chanceMap: static (NPC npc) =>
                    npc.SocialAnxiety == NPC.shy
                    && npc.Optimism == NPC.negative
                    && !npc.Name.Equals("George", StringComparison.OrdinalIgnoreCase)
                        ? 0.6
                        : 0.0,
                map: "IslandSouthEast",
            ...
            // pier points
            new PossibleIslandActivity(
                possiblepoints: [new Point(22, 43), new Point(22, 40)],
