...
internal readonly struct PossibleIslandActivity
{
    ...
    )
    {
        ...
        {
            schedule_animation = this.animation.StartsWith("square_")
                ? this.animation
                : $"{character.Name.ToLowerInvariant()}_{this.animation}";
        }
        // avoid repeating assignments.
        if (
            this.IsAnimationUnique()
            && lastAssignment?.TryGetValue(character, out string? lastanimation) == true
            && schedule_animation == lastanimation
        )
        {
            return null;
        }
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    private bool IsAnimationUnique() => !string.IsNullOrEmpty(this.animation) && !this.animation.StartsWith("square_");
}
