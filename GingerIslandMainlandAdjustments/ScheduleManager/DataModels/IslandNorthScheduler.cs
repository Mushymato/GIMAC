        ...
        if (explorers.Count > 0)
        {
            ...
            foreach (NPC explorer in explorers)
            {
                SchedulePoint firstPoint =
                    ...
                    ); // this little hackish thing makes them face in different directions.
            ...
            foreach (NPC explorer in explorerList)
            {
                ...
                new SchedulePoint(
                    ...
                    direction: explorerIndex
                ).AppendToStringBuilder(schedules[explorer]);
            }
            ...
            foreach (NPC explorer in explorerList)
            {
                ...
                sb.Append('/').AppendCorrectRemainderSchedule(explorer, out _);

                string renderedSchedule = sb.ToString();
                sb.Clear();

                Globals.ModMonitor.DebugOnlyLog(
                    $"Calculated island north schedule for {explorer.Name}: {renderedSchedule}"
                );
