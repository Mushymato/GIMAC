...
internal static class AssetEditor
{
    ...
    private static readonly PerScreen<TickCache<bool>> HasSeenNineHeart =
        new(static () => new(() => Game1.player?.eventsSeen?.Contains("503180") == true));

    private static readonly PerScreen<TickCache<bool>> HasSeenPamEvent =
        new(static () => new(() => Game1.player?.eventsSeen?.Contains(PAMEVENT) == true));
    ...
    private static readonly Dictionary<string, Action<IAssetData>> DialoguesToEdit =
    ...
    internal static void Edit(AssetRequestedEventArgs e)
    {
        ...
        else if (e.NameWithoutLocale.IsEquivalentTo(dataMail))
        {
            ...
            HasSeenNineHeart.Value.GetValue()
            && !HasSeenPamEvent.Value.GetValue()
            && e.NameWithoutLocale.IsEquivalentTo(dataEventsSeedshop)
        )
        {
        ...
        else if (e.NameWithoutLocale.StartsWith("Characters/schedules/", false, false))
        {
            ...
            e.NameWithoutLocale.StartsWith(Dialogue, false, false)
            && Game1.getLocationFromName("IslandSouth") is IslandSouth island
            && island.resortRestored.Value
        )
        {
    ...
    private static void CheckSpringSchedule(IAssetData e)
    {
        ...
        if (!editor.Data.ContainsKey("spring") && !editor.Data.ContainsKey("default"))
        {
            ...
            Globals.ModMonitor.LogOnce(
                $"Found NPC {character} without either a spring or default schedule. This may cause issues.",
                LogLevel.Info
            );
            ...
            if (!Failed.Contains(character))
            {
                try
                {
                    Dictionary<string, string> original = contentManager.Load<Dictionary<string, string>>(
                        e.NameWithoutLocale.BaseName
                    );
                    if (original.TryGetValue("spring", out string? data))
                    {
                        Globals.ModMonitor.LogOnce(
                            $"Original spring schedule found for {character}: {data}. Adding back",
                            LogLevel.Info
                        );
                    ...
                    {
                        Globals.ModMonitor.LogOnce(
                            $"Original default schedule found for {character}: {@default}. Adding back",
                            LogLevel.Info
                        );
    ...
    private static void EditPhone(IAssetData e)
    {
        ...
        foreach (
            string key in new string[]
            {
                ...
                "Pam_Bus_2",
                "Pam_Bus_3",
                "Pam_Voicemail_Island",
                "Pam_Voicemail_Doctor",
                "Pam_Voicemail_Other",
                "Pam_Voicemail_Bus",
                "Pam_Bus_Late",
            }
    ...
    {
        IAssetDataForDictionary<string, string>? editor = e.AsDictionary<string, string>();
        editor.Data[PAMMAILKEY] =
            $"{I18n.Pam_Mail_Text()}^^   --{NPCCache.GetByVillagerName("Pam")?.displayName ?? I18n.Pam()}[#]{I18n.Pam_Mail_Title()}";
    }
    ...
    private static void EditSeedShopEvent(IAssetData e)
    {
        ...
        editor.Data["atravita_GIMA_PamInsulted"] =
            $"friendship Pam -250/emote Pam 12/speak Pam \"{I18n._999Pam99()}\"/fade/viewport -100 -100/end invisible Pam";
    }
