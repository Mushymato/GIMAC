...
internal static class AssetLoader
{
        ...
        foreach (string? specialChar in Globals.GameContentHelper.Load<Dictionary<string, string>>(assetLocation).Keys)
        {
            ...
            else
            {
                Globals.ModMonitor.Log(
                    I18n.Assetmanager_SpecialcharNotFound(specialCharacterType, specialChar),
                    LogLevel.Debug
                );
            }
        }
    ...
    internal static Dictionary<string, HashSet<NPC>> GetCharacterGroup(SpecialGroupType specialGroupType)
    {
        ...
        foreach (string? groupname in data.Keys)
        {
            ...
            HashSet<NPC> group = [];
            foreach (
                SpanSplitEntry character in data[groupname]
                    .StreamSplit(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            )
            {
                ...
                {
                    Globals.ModMonitor.Log(
                        I18n.Assetmanager_GroupcharNotFound(specialGroupType, charname, groupname),
                        LogLevel.Debug
                    );
                }
            }
    ...
    internal static Dictionary<NPC, string[]> GetExclusions()
    {
        ...
        Dictionary<string, string> data = Globals.GameContentHelper.Load<Dictionary<string, string>>(
            ExclusionLocations
        );
    ...
    {
        Dictionary<string, string> defaultgroups = Globals.ModContentHelper.Load<Dictionary<string, string>>(
            "assets/defaultGroupings.json"
        );
        ...
        if (
            defaultgroups.TryGetValue("barfolk", out string? value)
            && Game1.getAllFarmers().Any((Farmer farmer) => farmer.eventsSeen.Contains("99210002"))
        )
        {
