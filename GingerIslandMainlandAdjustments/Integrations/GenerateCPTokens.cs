...
internal class GenerateCPTokens
{
    ...
    internal static void AddTokens(IManifest manifest)
    {
        ...
        //TODO - is this the token I want, or do I want is the island truly open for business?
        api.RegisterToken(
            manifest,
            "IslandOpen",
            () =>
            {
                if (
                    (Context.IsWorldReady || SaveGame.loaded is not null)
        ...
        api.RegisterToken(
            manifest,
            "Bartender",
            () => GIScheduler.Bartender is null ? null : new[] { GIScheduler.Bartender.displayName }
        );
        api.RegisterToken(
            manifest,
            "Musician",
            () => GIScheduler.Musician is null ? null : new[] { GIScheduler.Musician.displayName }
        );
        api.RegisterToken(
            manifest,
            "Islanders",
