...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    internal static void Initialize(IModHelper helper, IMonitor monitor, IManifest manifest)
    {
        ...
        Globals.Config = AtraSharedUtils.GetConfigOrDefault<ModConfig>(helper, monitor);

        UtilitySchedulingFunctions = new(monitor: Globals.ModMonitor, translation: Globals.Helper.Translation);
    }
    ...
    internal static bool GetIsChildToNPC()
    {
        ...
        if (
            Type.GetType("ChildToNPC.ModEntry, ChildToNPC")?.GetMethod("IsChildNPC", new Type[] { typeof(Character) })
            is MethodInfo childToNPCMethod
        )
        {
