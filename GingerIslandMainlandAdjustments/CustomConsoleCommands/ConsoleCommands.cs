...
internal static class ConsoleCommands
{
    ...
    internal static void Register(ICommandHelper commandHelper)
    {
        commandHelper.Add(
            ...
            callback: static (string command, string[] args) =>
                Globals.ModMonitor.Log(
                    I18n.BaseCommand()
    ...
    private static void QueueNPC(string command, string[] args)
    {
        ...
        {
            Globals.ModMonitor.Log(
                "NPCs can only be queued by the main player, and from within a save.",
                LogLevel.Info
            );
            return;
        }
    ...
    private static void DisplaySchedule(NPC npc, LogLevel level)
    {
        ...
        else
        {
            ...
            {
                Globals.ModMonitor.Log(
                    $"\t{npc.Name} lacks a npc.dayScheduleName.Value yet claims to be following a schedule today.",
                    LogLevel.Error
                );
            }
            ...
            {
                if (
                    Globals.UtilitySchedulingFunctions.TryFindGOTOschedule(
                        npc,
                        SDate.Now(),
                        npc.getMasterScheduleEntry(npc.dayScheduleName.Value),
                        out string schedulestring
                    )
                )
                {
            ...
            {
                Globals.ModMonitor.Log(
                    $"\t{npc.Name} claims to be using {npc.dayScheduleName.Value} but that was not found!",
                    LogLevel.Error
                );
            }
        }
        ...
        {
            Globals.ModMonitor.Log(
                $"Something very odd has happened to the schedule of {npc.Name} - it appears to have been nulled since generation",
                LogLevel.Error
            );
            if (!npc.CanSocialize)
            {
                ...
                Globals.ModMonitor.Log(
                    $"\t{npc.Name} appears to be antisocial: they {(antisocial.ContainsKey(npc.Name) ? "are" : "aren't")} registered with AntisocialNPCs.",
                    LogLevel.Info
                );
            }
            return;
        ...
        keys.Sort();
        StringBuilder sb = new(keys.Count * 100);
        foreach (int key in keys)
        {
            ...
            sb.Append('\t')
                .Append(key)
                .Append(' ')
                .Append(schedulePathDescription.targetLocationName)
                .Append(' ')
                .Append(schedulePathDescription.targetTile)
                .Append(": ");
            sb.AppendJoin(", ", schedulePathDescription.route).AppendLine();
            sb.Append("\t\t")
                .Append(I18n.DisplaySchedule_ExpectedTime())
                .Append(expectedRouteTime)
                .AppendLine(I18n.DisplaySchedule_ExpectedTime_Minutes());
            sb.Append("\t\t")
                .Append(I18n.DisplaySchedule_ExpectedArrival())
                .AppendLine(Utility.ModifyTime(key, expectedRouteTime).ToString());
            sb.Append("\t\t")
                .Append(I18n.DisplaySchedule_Direction())
                .AppendLine(schedulePathDescription.facingDirection.ToString());
            sb.Append("\t\t")
                .Append(I18n.DisplaySchedule_Animation())
                .AppendLine(schedulePathDescription.endOfRouteBehavior);
            sb.Append("\t\t")
                .Append(I18n.DisplaySchedule_Message())
                .AppendLine(schedulePathDescription.endOfRouteMessage);
        }
        sb.AppendLine();
    ...
    private static void ConsoleGetLocations(string command, string[] args)
    {
        ...
        if (
            Globals
                .ReflectionHelper.GetField<Dictionary<string, List<LocationWarpRoute>>>(
        ...
        {
            Globals.ModMonitor.Log(
                $"Looking for {string.Join(", ", args)} in routesFromLocationToLocation",
                LogLevel.Info
            );
        }
        Func<string[], bool> filter =
            args.Length == 0 ? (_) => true : (string[] loclist) => loclist.Any(a => locations_to_find.Contains(a));
