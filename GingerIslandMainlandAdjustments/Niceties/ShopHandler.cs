...
internal static class ShopHandler
{
    ...
    internal static void HandleSandyShop(ButtonPressedEventArgs e)
    {
        ...
        if (
            ...
            || sandyHouse.getCharacterFromName("Sandy") is not null // Sandy has not left the building.
            || !AtraSharedUtils
                .YieldSurroundingTiles(Globals.Helper.Input.GetCursorPosition().GrabTile)
                .Any(
                    predicate: (Point v) =>
                        sandyHouse.doesTileHaveProperty(v.X, v.Y, "Action", "Buildings")?.Contains("Buy") == true
                )
        )
        {
            return;
    ...
    internal static void HandleWillyShop(ButtonPressedEventArgs e)
    {
        if (
            HandlingShop.Value
            || Game1.currentLocation is not FishShop fishShop
            || !Game1.IsVisitingIslandToday("Willy")
            || fishShop.getCharacterFromName("Willy") is not null
        )
        {
            return;
        }
        ...
        if (
            !fishShop.doesTileHaveProperty((int)cursorTile.X, (int)cursorTile.Y, "Action", "Buildings")?.Contains("Buy")
            == true
        )
        {
            return;
    ...
    internal static void AddBoxToShop(WarpedEventArgs e)
    {
        if (
            (
                Game1.IsVisitingIslandToday("Sandy")
                && e.NewLocation.Name.Equals("SandyHouse", StringComparison.OrdinalIgnoreCase)
                && e.NewLocation.getCharacterFromName("Sandy") is null
            ) // Sandy has left already
            || (
                e.NewLocation is FishShop fishShop
                && Game1.IsVisitingIslandToday("Willy")
                && fishShop.getCharacterFromName("Willy") is null
            )
        )
        {
            Vector2 tile = e.NewLocation is FishShop ? new Vector2(5f, 5f) : new Vector2(2f, 6f); // default location of shop.
            foreach (Vector2 v in AtraSharedUtils.YieldAllTilesBuildings(e.NewLocation))
            { // find the shop tile - a mod may have moved it.
                if (
                    e.NewLocation.doesTileHaveProperty((int)v.X, (int)v.Y, "Action", "Buildings")?.Contains("Buy")
                    == true
                )
                {
                    tile = v;
            ...
            // add box
            e.NewLocation.temporarySprites.Add(
