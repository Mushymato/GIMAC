...
public static class MultiplayerSharedState
{
    ...
    private static readonly PerScreen<TickCache<bool>> HasSeenEvent =
        new(static () => new(static () => Game1.player.eventsSeen.Contains(AssetEditor.PAMEVENT)));
    ...
    private static void PostfixGetMasterSchedule(NPC __instance)
    {
        try
        {
            if (
                Context.IsMainPlayer
                && __instance?.Name.Equals("Pam", StringComparison.OrdinalIgnoreCase) == true
