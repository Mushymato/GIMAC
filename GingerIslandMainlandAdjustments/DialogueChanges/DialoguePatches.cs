...
internal static class DialoguePatches
{
    ...
    { // __0 = heartlevel, as int. __1 = whether or not to have a season prefix?
        try
        {
            ...
            if (
                __instance.currentLocation is IslandLocation
                && GIScheduler.CurrentAdventurers?.Contains(__instance) == true
            )
            {
            ...
            if (
                __instance.getSpouse() is Farmer spouse
                && spouse == Game1.player
    ...
    private static void AppendArrival(NPC __instance)
    {
        try
        {
            ...
            if (
                TalkedToToday.Contains(__instance.Name)
                && __instance.TryApplyMarriageDialogueIfExisting(
                    "GIReturn_Talked_" + __instance.Name,
                    clearOnMovement: true
                )
            )
            {
                ...
                __instance.TryApplyMarriageDialogueIfExisting("GIReturn_" + __instance.Name, clearOnMovement: true)
            )
            {
            ...
            else
            {
                ...
                Dialogue dialogue =
                    Game1.player.getFriendshipHeartLevelForNPC(__instance.Name) > 9
                        ? new Dialogue(
                            __instance,
                            null,
                            I18n.GIReturnDefaultHappy(__instance.getTermOfSpousalEndearment())
                        )
