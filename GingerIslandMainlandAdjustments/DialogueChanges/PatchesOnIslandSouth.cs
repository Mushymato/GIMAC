...
internal static class PatchesOnIslandSouth
{
    ...
    {
        if (
            GIScheduler.CurrentVisitingGroup?.Contains(character) != true
            || GIScheduler.CurrentGroup is not string group
        )
        {
            return;
        }
        ...
        if (
            ...
            || (
                !character.shouldWearIslandAttire.Value
                && Game1.timeOfDay < 1730
                && IslandSouth.HasIslandAttire(character)
            )
        )
        {
